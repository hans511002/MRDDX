...
/**
 * Reducer类 Copyrights @ 2012,Tianyuan DIC Information Co.,Ltd. All rights
 * reserved.
 * 
 * @author wanghao
 * @version v1.0
 ...
 */
public class DBPartitionReducer extends Reducer<DBRecord, DBRecord, DBRecord, DBRecord> implements IDBMonitor,
		IDBSenior {
	// 日志对象
	public static final Log LOG = LogFactory.getLog(DBPartitionReducer.class);
	...
	private Map<String, Object> tempMapValue = new HashMap<String, Object>();
	...
	protected void reduce(DBRecord key, Iterable<DBRecord> values, Context context) throws IOException,
		...
		try {
			...
			for (DBRecord _nrow : values) {
				...
				this.tempMapValue.putAll(_nrow.getRow());
				this.tempMapValue.putAll(key.getRow());
				List<Map<String, Object>> listVal = IRemotePlugin.mapPlugin(this.remotePlugin, tempMapValue);
				...
				for (int x = 0; x < listVal.size(); x++) {
					...
					for (int i = 0; i < this.destFieldNames.length; i++) {
						...
						if (method == 0) {
							sum.putData(fieldName, dbrecord.getData(fieldName));
							continue;
						}
						...
						case 2: // AVG
							...
							// StringUtil.doubleToString(d + objv,
							// StringUtil.DOUBLE_FORMAT_8Decimal)
							break;
						case 3: // MIN
			...
			// 打印日志记录
			this.debugInfo("输出:", gps[0], gps[1], 0);
		} catch (Exception e) {
			// 异常设置任务节点失败
			if (this.isNeedZKMonitor && !this.inputMapEnd && null != this.monitorZKNode) {
				try {
