...
public class HiveSequenceFileRecordWriter<K extends FileWritable, V> extends RecordWriter<K, V> {
	// 日志对象
	private static final Log LOG = LogFactory.getLog(HiveSequenceFileRecordWriter.class);
	 ...
	 * @throws IOException
	 */
	public HiveSequenceFileRecordWriter(TaskAttemptContext context, FileOutputFormat inputOutputFormat)
			throws IOException {
		...
		this.lstPartitionOutOS = new HashMap<String, SequenceFile.Writer>();
		this.lstPartitionFilePath = new HashMap<String, String>();
		this.lstPartitionFileCount = new HashMap<String, Long>();
		this.keyClass = context.getOutputKeyClass();
		this.valueClass = context.getOutputValueClass();
		...
		if (null != this.partitionField && this.partitionField.length > 0 &&
			...
			if (!isCompressed) {
				...
				this.out = SequenceFile.createWriter(fs, job, file, keyClass, valueClass, compressionType, codec,
						context);
			} else {
				// 需要压缩
				// 获取压缩标识
				compressionType = hiveConf.getOutputCompressionType();
	...
	public void close(TaskAttemptContext context) throws IOException, InterruptedException {
		...
		for (String key : this.lstPartitionFilePath.keySet()) {
			...
			SequenceFile.Writer outos = this.lstPartitionOutOS.remove(realPath);
			if (null == outos) {
				continue;
			}
			if (null != outos) {
		...
		this.lstPartitionFilePath.clear();
		// 删除临时目录下创建分区目录
	...
	public void write(K key, V value) throws IOException, InterruptedException {
		...
		if (!nullKey && key instanceof DBRecord) {
			if (null == this.partitionField || this.partitionField.length <= 0) {
				System.out.println(StringUtil.parseArrayToString(partitionField, ","));
				...
				SequenceFile.Writer filterFSOStream = this.lstPartitionOutOS.get(realFilePath);
				long count = 0;
	...
	private void closeOutputStream(SequenceFile.Writer filterFSOStream) throws IOException {
	...
	private void initOutputStream(String realFilePath) throws IOException {
		...
		if (!isCompressed) {
			...
			FileSystem fs = file.getFileSystem(job);
			SequenceFile.Writer out = SequenceFile.createWriter(fs, job, file, keyClass, valueClass, compressionType,
					codec, null);
	...
	private String getFilePath(K key) {
		...
		for (int i = 0; i < partition.length; i++) {
			System.out.println(i + "  partition=" + partition[i]);
		}
