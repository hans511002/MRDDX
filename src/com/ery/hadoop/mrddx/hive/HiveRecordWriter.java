...
public class HiveRecordWriter<K extends FileWritable, V> extends RecordWriter<K, V> {
	// 日志对象
	private static final Log LOG = LogFactory.getLog(HiveRecordWriter.class);
	...
	// 分区输出流列表,实际的文件目录与输出流
	private Map<String, DataOutputStream> lstPartitionOutOS;
	// 分区输出流列表,实际的文件目录与记录数
	 ...
	 */
	public HiveRecordWriter(TaskAttemptContext context, FileOutputFormat inputOutputFormat) throws IOException,
		...
		this.lstPartitionOutOS = new HashMap<String, DataOutputStream>();
		...
		if (null != this.partitionField && this.partitionField.length > 0 &&
			...
			HiveConfiguration hiveConf = new HiveConfiguration(context.getConfiguration());
			boolean isCompressed = hiveConf.getOutputHiveCompress();
			// 无需压缩
			if (!isCompressed) {
				// Path file = FileOutputFormat.getTaskOutputPath(context);
				String name = context.getConfiguration().get(HiveConfiguration.OUTPUT_HIVE_ORDER_FILENAME, "PART");
				...
				// extension));
				FileSystem fs = FileSystem.get(context.getConfiguration());// file.getFileSystem(job);
				this.out = fs.create(file, context);
			} else {// 需要压缩
					// 获取压缩标识
				String compresseCodec = hiveConf.getOutputHiveCompressCodec();
				CompressionCodec codec = HDFSUtils.getCompressCodec(compresseCodec, context.getConfiguration());
				String name = context.getConfiguration().get(HiveConfiguration.OUTPUT_HIVE_ORDER_FILENAME, "PART");
				...
				this.out = new DataOutputStream(codec.createOutputStream(fileOut));
			}
		}
	}
	...
	public void close(TaskAttemptContext context) throws IOException, InterruptedException {
		...
		for (String key : this.lstPartitionFilePath.keySet()) {
			...
			DataOutputStream outos = this.lstPartitionOutOS.remove(realPath);
			this.closeOutputStream(outos);
	...
	public void write(K key, V value) throws IOException, InterruptedException {
		...
		try {
			if (null == key || !(key instanceof DBRecord)) {
			...
			if (this.fieldNames == null) {
				...
				for (int i = 0; i < fieldNames.length; i++) {
					if (i > 0) {
						this.out.writeChars(this.fieldSeparator);
					}
					this.out.writeChars(fieldNames[i]);
				}
				this.out.write(this.rowSeparator.getBytes());
			}
			...
			if (null == this.partitionField || this.partitionField.length <= 0) {
				...
				((DBRecord) key).write(this.out, this.fieldNames, this.fieldSeparator.getBytes(),
						this.rowSeparator.getBytes(), this.isNotPartitionsFistRow);
				...
				DataOutputStream filterFSOStream = this.lstPartitionOutOS.get(realFilePath);
				long count = 0;
				...
				((DBRecord) value).write(filterFSOStream, this.fieldNames, this.fieldSeparator.getBytes(),
						this.rowSeparator.getBytes(), isFirstRow);
			...
			e.printStackTrace();
			MRLog.error(LOG, "hive write data error!");
			throw new IOException(e.getMessage());
		}
	}

	private void closeOutputStream(DataOutputStream filterFSOStream) {
		if (null == filterFSOStream) {
			return;
		}
		System.out.println("close filterFSOStream=" + filterFSOStream);
		try {
			filterFSOStream.flush();
		} catch (IOException e) {
			e.printStackTrace();
		}
		try {
	...
	private void initOutputStream(String realFilePath) throws IOException {
		...
		// 无需压缩
		if (!isCompressed) {
			DataOutputStream filterFSOStream = HDFSUtils.getFileOStream(job, realFilePath, this.orderWriteBufferSize);
			this.lstPartitionOutOS.put(realFilePath, filterFSOStream);
		...
		DataOutputStream fileOut = HDFSUtils.getFileOStream(job, realFilePath, this.orderWriteBufferSize);
		DataOutputStream dos = new DataOutputStream(codec.createOutputStream(fileOut));
		this.lstPartitionOutOS.put(realFilePath, dos);
		this.lstPartitionFileCount.put(realFilePath, 0l);
	}
