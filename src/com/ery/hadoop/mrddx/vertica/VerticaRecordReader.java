...
public class VerticaRecordReader<T extends DBWritable> extends DBRecordReader<T> {

	public VerticaRecordReader(InputSplit split, Class<T> inputClass, Configuration conf, Connection conn,
	...
	protected ResultSet executeQuery(DBInputFormat.DBInputSplit split) throws SQLException {
		if (this.split instanceof VerticaInputSplit) {
			VerticaInputSplit vs = ((VerticaInputSplit) this.split);
			MRLog.consoleDebug(LOG, "VerticaInputSplit: " + vs.inputQuery);
	...
	protected String getSelectQuery() {
		if (this.split instanceof VerticaInputSplit) {
			return ((VerticaInputSplit) this.split).inputQuery;
		} else {
			return null;
		}
	}
