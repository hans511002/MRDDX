...
public class TextSplitter extends BigDecimalSplitter {
	 ...
	 * splits for strings beginning with each letter, etc. If a user has
	 * provided us with the strings "Ham" and "Haze", however, we need to create
	 * splits that differ in the third letter. The algorithm used is as follows:
	...
	public List<InputSplit> split(Configuration conf, ResultSet results, String colName) throws SQLException {
		...
		for (int i = 1; i < splitStrings.size(); i++) {
			...
			if (i == splitStrings.size() - 1) {
				...
				splits.add(new DataDrivenDBInputFormat.DataDrivenDBInputSplit(lowClausePrefix + start + "'", colName
						+ " <= '" + end + "'"));
			} else {
	 ...
	 *             SQL异常
	 */
	private List<String> split(int numSplits, String minString, String maxString, String commonPrefix)
			throws SQLException {
