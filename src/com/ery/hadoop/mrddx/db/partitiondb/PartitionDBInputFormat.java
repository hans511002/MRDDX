...
public class PartitionDBInputFormat<T extends DBWritable> extends DBInputFormat<T> implements Configurable,
		IHandleFormat {
	private static final Log LOG = LogFactory.getLog(PartitionDBInputFormat.class);
	...
	public static final String INPUT_SPLIT_PARTITION_HOSTS = "input.split.partition.hosts";
	public static final String COBAR_SPLIT_PARTITION_MYSQLS_URL = "input.mr.mapred.jdbc.partition.urls";

	private String inputQuery = null;
	private String params = null;

	public PartitionDBInputFormat() {
	}

	public PartitionDBInputFormat(String query, String params) {
		inputQuery = query;
		this.params = params;
	}
	...
	public RecordReader<LongWritable, T> createRecordReader(InputSplit split, TaskAttemptContext context)
		...
		try {
			Connection conn = this.getConnection((PartitionDBInputSplit) split);
			MRLog.info(LOG, logId, "RecordReader:RDBRecordReader");
			return new PartitionDBRecordReader<T>(split, inputClass, this.conf, conn, this.dbConf, this.conditions,
	...
	public Connection getConnection(PartitionDBInputSplit split) throws Exception {
		return getConnection(split.hostUrl);
	}

	public Connection getConnection(String hostUrl) throws Exception {
		try {
			if (null == this.connection) {
				String url = hostUrl;
				String driverName = this.dbConf.getDriverName(url, true);
				String user = this.dbConf.getInputJDBCUserName();
				String pwd = this.dbConf.getInputJDBCPassword();
				if (null == driverName || url == null) {
					throw new SQLException("连接地址不正确,url:" + url + " driverName:" + driverName + " user:" + user +
							" pwd:" + pwd);
				}
	...
	public static void setInput(Job job, String inputQuery) {
		job.setInputFormatClass(PartitionDBInputFormat.class);
	...
	public List<InputSplit> getSplits(JobContext context) throws IOException {
		...
		String hosts[] = conf.getStrings(INPUT_SPLIT_PARTITION_HOSTS);

		String hostUrl = conf.get(COBAR_SPLIT_PARTITION_MYSQLS_URL);
		Map<String, String> partitionUrlMap = PartitionDBRecordWriter.jsonMap.readValue(hostUrl, Map.class);

		String[] hostUrls = new String[hosts.length];

		for (int i = 0; i < hostUrls.length; i++) {
			hostUrls[i] = partitionUrlMap.get(hosts[i]);
			if (hostUrls[i] == null) {
				throw new IOException("配置错误，数据源主机[" + hosts[i] + "]在数据池" + partitionUrlMap + "中不存在");
			}
		}
		...
		for (int h = 0; h < hostUrls.length; h++) {
			...
			if (isSelectSQL) {
				...
				try {
					...
					while (rs.next()) {
						...
						if (macroParamIndex != null && macroParamIndex.length > 0) {
							...
							splits.add(new PartitionDBInputSplit(hostUrls[h], tmpSql, segmentParams));
						} else {
							splits.add(new PartitionDBInputSplit(hostUrls[h], inputQuery, segmentParams));
						}
					}
				} catch (Exception e) {
				...
				for (int i = 1; i < numSplits; i++) {
					splits.add(new PartitionDBInputSplit(hostUrls[h], inputQuery, start, end));
				...
				if (count > 0) {
					splits.add(new PartitionDBInputSplit(hostUrls[h], inputQuery, start, count));
				}
			} else {
				splits.add(new PartitionDBInputSplit(hostUrls[h], inputQuery, 0, 0));
			}
		}
