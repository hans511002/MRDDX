...
public class PartitionDBRecordReader<T extends DBWritable> extends DBRecordReader<T> {

	public PartitionDBRecordReader(InputSplit split, Class<T> inputClass, Configuration conf, Connection conn,
	...
	protected ResultSet executeQuery(DBInputFormat.DBInputSplit split) throws SQLException {
		if (this.split instanceof PartitionDBInputSplit) {
			PartitionDBInputSplit vs = ((PartitionDBInputSplit) this.split);
	...
	protected String getSelectQuery() {
		if (this.split instanceof PartitionDBInputSplit) {
			return ((PartitionDBInputSplit) this.split).inputQuery;
		} else {
			return null;
		}
	}
