...
import com.ery.hadoop.mrddx.remote.FileAttributePO;
...
public class HDFSUtils {
	...
	public static FileSystem getFileSystem(String ip, int port) {
		...
		config.set(MRConfiguration.FS_DEFAULT_NAME, url);
		try {
	 ...
	 * @param dstPrefix
	 *            a prefix name of that after moved file/directory name, it can
	 *            be null or empty
	 * @throws IOException
	 */
	 ...
	 * @return 唯一的job名称
	 */
	public static String getJobName(MRConfiguration mrconf) {
		String jobName = StringUtil.changeBlankToUnderline(mrconf.getJobName());
		return jobName + StringUtil.getUniqueId();
	}
	...
	public static OutputStream getCompressOutputStream(String outFilePath, Object outSystem, int compress)
		...
		if (compress == 0) {
			MRLog.systemOut(MRLog.get6Space() + "获取普通流");
			return ((FileSystem) outSystem).create(new Path(outFilePath));
		} else if (compress == 1) {
			MRLog.systemOut(MRLog.get6Space() + "获取GZ压缩流");
	...
	/**
	 * 获取hdfs的解压缩后的字节流 1、非压缩文件
	 * 2、单个文件压缩（或单个文件打包，单个文件打包压缩）(Gzip,snnapy,bzip2,Default)
