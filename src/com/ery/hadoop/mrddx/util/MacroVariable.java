...
/**
 * 系统默认提供的宏变量; N_YYYY 当前时间-年(四位) N_YY 当前时间-年(后两位) N_MM 当前时间-月(两位) N_M
 * 当前时间-月(当第一位为0时，只取第二位，否则取两位) N_DD 当前时间-天(两位)"); N_D
 * 当前时间-天(当第一位为0时，只取第二位，否则取两位) N_HH 当前时间-小时(两位) N_MI 当前时间-分钟(两位) N_SS 当前时间-秒(两位)
 * N_YYYYMMDD 当前时间-年月日(八位) N_YYYYMM 当前时间-年月(六位)"); N_YYYYMMP 当前时间的前一个月-年月(六位)
...
public class MacroVariable {
	...
	public static String getCurrentN_DATE() {
		Date date = new Date();
		return StringUtil.dateToString(date, DATE_FORMAT_TYPE);
	}
